(function (win, $, lib) {
    ...
    function levelsChoices(obj) {
        ...
        this.boxname = (obj.boxname != null) ? obj.boxname : '.wrap';  //传的参数表示我要runder到哪个div里面去
    ...
    levelsChoices.prototype = {
        ...
        removeAndAddClass: function (element, type) {
            var that=this;
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if ($($(that.boxname+' li')[i]).attr('type') == type) {
                    $($(that.boxname+' li')[i]).find("span").removeClass("selected");
                }
            }
        ...
        displayNone: function (type) {
            ...
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if ($($(that.boxname+' li')[i]).attr('type') == type) {
                    $($(that.boxname+' li')[i]).css('display', 'none');
                }
            }
        },
        ...
        displayForthType: function (id) {
            ...
            that.displayNone(4);
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if ($($(that.boxname+' li')[i]).attr('type') == 4) {
                    if ($($(that.boxname+' li')[i]).attr('parent').split("-")[0] == id) {
                        $($(that.boxname+' li')[i]).css('display', 'block');
                    }
                }
            }
        },
        ...
        displayNextType: function (type, path) {
            ...
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if ($($(that.boxname+' li')[i]).attr("type") == type) {
                    if ($($(that.boxname+' li')[i]).attr('parent') == path) {
                        $($(that.boxname+' li')[i]).css('display', 'block');
                        arr.push($($(that.boxname+' li')[i]));
                    }
                }
            }
            return arr[0];
        ...
        search_byId: function (id, type) {
            var that=this;
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if ($($(that.boxname+' li')[i]).attr('parent').split("-")[type - 1] == id && $($('li')[i]).attr('type') > type) {
                    $($(that.boxname+' li')[i]).find("i").addClass('ok');
                    if ($($(that.boxname+' li')[i]).attr('type') == 4) {
                        $($(that.boxname+' li')[i]).find("span").addClass('selected');
                    }
                }
            }
        },
        ...
        remove_byId: function (id, type) {
            var that=this;
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if ($($(that.boxname+' li')[i]).attr('parent').split("-")[type - 1] == id && $($(that.boxname+' li')[i]).attr('type') > type) {
                    $($(that.boxname+' li')[i]).find("i").removeClass('ok');
                    $($(that.boxname+' li')[i]).find("i").removeClass('sub');
                    if ($($(that.boxname+' li')[i]).attr('type') == 4) {
                        $($(that.boxname+' li')[i]).find("span").removeClass('selected');
                    }
                }
        ...
        getReturnArr: function () {
            ...
            var obj={};
            for(var i=0,len=$(that.boxname+' li').length;i<len;i++){
                idnum=$($(that.boxname+' li')[i]).attr('idnum');
                value=$($(that.boxname+' li')[i]).attr('value');
                path=$($(that.boxname+' li')[i]).attr('path');
                type=$($(that.boxname+' li')[i]).attr('type');
                if($($(that.boxname+' li')[i]).find('i').hasClass('ok')){
                    if(parseInt($($(that.boxname+' li')[i]).attr('type'))==1){
                        obj = {
                            industryType: idnum,
                            name: value,
                            path: path,
                    ...
                    for(var j=0,length=$(that.boxname+' li').length;j<length;j++){
                        if(($($(that.boxname+' li')[j]).attr('path') == $($(that.boxname+' li')[i]).attr('parent')) && $($(that.boxname+' li')[j]).find('i').hasClass('ok')){

                        }else if(($($(that.boxname+' li')[j]).attr('path') == $($(that.boxname+' li')[i]).attr('parent')) && !$($(that.boxname+' li')[j]).find('i').hasClass('ok')){
                            switch(parseInt($($(that.boxname+' li')[i]).attr('type'))){
                                case 2 :
                                    obj = {
        ...
        yulan: function (props) {
            ...
            for (var i = 0, l1 = props.length; i < l1; i++) {
                for (var j = 0, l2 = props[i].length; j < l2; j++) {
                    for (var k = 0, l3 = $(that.boxname+' li').length; k < l3; k++) {
                        if (props[i][j].path == $($(that.boxname+' li')[k]).attr('path')) {
                            $($(that.boxname+' li')[k]).find('i').click();
                        }
                    }
                }
            }
        ...
        addSubtoFather : function(parent){
           var that=this;
            for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                if($($(that.boxname+' li')[i]).attr('path')==parent){
                    $($(that.boxname+' li')[i]).find('i').addClass('sub');
                    if($($(that.boxname+' li')[i]).attr('parent') && ($($(that.boxname+' li')[i]).attr('parent') != $($(that.boxname+' li')[i]).attr('path'))){
                        that.addSubtoFather($($(that.boxname+' li')[i]).attr('parent'));
                    }
                }
            }
        },
        ...
        changeFather: function (path, parent) {
            ...
            if (path != parent) {
                ...
                for (var i = 0, len = $('li').length; i < len; i++) {
                    //记录传入的节点所有的兄弟
                    if ($($(that.boxname+' li')[i]).attr('parent') == parent && $($(that.boxname+' li')[i]).attr('type') != 1) {
                        childArr.push($(that.boxname+' li')[i]);
                    }
                }
                var sum = 0;
                ...
                for (var i = 0, len = $(that.boxname+' li').length; i < len; i++) {
                    //第一层改变方式
                    if ($($(that.boxname+' li')[i]).attr('type') == 1 && $($(that.boxname+' li')[i]).attr('path') == parent) {
                        if (sum == childArr.length) {
                            $($(that.boxname+' li')[i]).find('i').removeClass('ok');
                            $($(that.boxname+' li')[i]).find('i').removeClass('sub');
                            $($(that.boxname+' li')[i]).find('i').addClass('ok');

                            that.changeFather($($(that.boxname+' li')[i]).attr('path'),$($(that.boxname+' li')[i]).attr('parent'));
